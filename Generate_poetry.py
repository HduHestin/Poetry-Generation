...
from Poetry_show import poetry_show
...
# 特定风格诗
def random_spec_poetry(string):
    poetry = poetry_gen.generate(style=string)
    poetry_show(poetry)


# 藏头诗
def random_hide_poetry(plist):
    poetry = poetry_gen.generate(head=plist)
    return poetry_show(poetry)
